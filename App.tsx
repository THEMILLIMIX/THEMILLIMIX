...
export default function App() {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        setCollabPasswords(config.collabPasswords);
      } catch (error) {
  ...
  // Function to update config on the server
  const updateConfigOnServer = async (newConfig: any) => {
    try {
  ...
  const handleAddCollabPassword = (pw: string, rate: number) => {
    if (pw && !collabPasswords.some(c => c.pw === pw)) {
      const newPasswords = [...collabPasswords, { pw, rate }];
      setCollabPasswords(newPasswords);
      updateConfigOnServer({ services, options, collabPasswords: newPasswords });
    }
  };

  const handlePriceUpdate = (id: string, newPrice: number) => {
    const newServices = services.map(s => s.id === id ? { ...s, price: newPrice } : s);
    const newOptions = options.map(o => o.id === id ? { ...o, price: newPrice } : o);
    setServices(newServices);
    setOptions(newOptions);
    updateConfigOnServer({ services: newServices, options: newOptions, collabPasswords });
  };

  const handleDeleteCollabPassword = (pw: string) => {
    const newPasswords = collabPasswords.filter(c => c.pw !== pw);
    setCollabPasswords(newPasswords);
    updateConfigOnServer({ services, options, collabPasswords: newPasswords });
  };
